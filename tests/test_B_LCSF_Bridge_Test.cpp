...
static bool compare_valid_cc2(const lcsf_valid_cmd_t *p1, const lcsf_valid_cmd_t *p2) {
    ...
    if ((p1->CmdId != LCSF_TEST_CMD_ID_CC2) || (p2->CmdId != LCSF_TEST_CMD_ID_CC2)) {
        DEBUG_PRINT("[tests]: CC2: Wrong command id: %d, %d\n", p1->CmdId, p2->CmdId);
        return false;
    }
    if ((p1->pAttArray == NULL) || (p2->pAttArray == NULL)) {
        DEBUG_PRINT("[tests]: CC2: Missing attributes\n");
        return false;
    }
    // SA1
    ...
    if (pA1->PayloadSize != pA2->PayloadSize) {
        DEBUG_PRINT("[tests]: CC2: Wrong SA4 size: %d, %d\n", pA1->PayloadSize, pA2->PayloadSize);
        return false;
    }
    if ((pA1->Payload.pData == NULL) || (pA2->Payload.pData == NULL)) {
        DEBUG_PRINT("[tests]: CC2: Missing SA4\n");
        return false;
    }
    if (memcmp(pA1->Payload.pData, pA2->Payload.pData, pA1->PayloadSize) != 0) {
        DEBUG_PRINT("[tests]: CC2: Different SA4\n");
        return false;
    }
    // SA5
    pA1 = &p1->pAttArray[4];
    pA2 = &p2->pAttArray[4];
    if ((pA1->Payload.pData == NULL) || (pA2->Payload.pData == NULL)) {
        DEBUG_PRINT("[tests]: CC2: Missing SA5\n");
        return false;
    }
    if (strcmp((char *)pA1->Payload.pData, (char *)pA2->Payload.pData) != 0) {
        DEBUG_PRINT("[tests]: CC2: Different SA5\n");
        return false;
    }
    // SA6
    pA1 = &p1->pAttArray[5];
    ...
    if ((pA1->Payload.pData == NULL) || (pA2->Payload.pData == NULL)) {
        if (pA1->Payload.pData != pA2->Payload.pData) {
            DEBUG_PRINT("[tests]: CC2: Mismatch absence of SA6\n");
            return false;
        }
    } else {
        if (memcmp(pA1->Payload.pData, pA2->Payload.pData, sizeof(uint8_t)) != 0) {
            DEBUG_PRINT("[tests]: CC2: Different SA6: %d, %d\n", *(uint8_t *)pA1->Payload.pData, *(uint8_t *)pA2->Payload.pData);
            return false;
        }
    }
    // SA7
    ...
    if ((pA1->Payload.pData == NULL) || (pA2->Payload.pData == NULL)) {
        if (pA1->Payload.pData != pA2->Payload.pData) {
            DEBUG_PRINT("[tests]: CC2: Mismatch absence of SA7: %p, %p\n", pA1->Payload.pData, pA2->Payload.pData);
            return false;
        }
    } else {
        if (memcmp(pA1->Payload.pData, pA2->Payload.pData, sizeof(uint16_t)) != 0) {
            DEBUG_PRINT("[tests]: CC2: Different SA7: %d, %d\n", *(uint16_t *)pA1->Payload.pData, *(uint16_t *)pA2->Payload.pData);
            return false;
        }
    }
    // SA8
    ...
    if ((pA1->Payload.pData == NULL) || (pA2->Payload.pData == NULL)) {
        if (pA1->Payload.pData != pA2->Payload.pData) {
            DEBUG_PRINT("[tests]: CC2: Mismatch absence of SA8\n");
            return false;
        }
    } else {
        if (memcmp(pA1->Payload.pData, pA2->Payload.pData, sizeof(uint32_t)) != 0) {
            DEBUG_PRINT("[tests]: CC2: Different SA8: %d, %d\n", *(uint32_t *)pA1->Payload.pData, *(uint32_t *)pA2->Payload.pData);
            return false;
        }
    }
    // SA9
    ...
    if ((pA1->Payload.pData == NULL) || (pA2->Payload.pData == NULL)) {
        if (pA1->Payload.pData != pA2->Payload.pData) {
            DEBUG_PRINT("[tests]: CC2: Mismatch absence of SA9\n");
            return false;
        }
    } else {
        if (pA1->PayloadSize != pA2->PayloadSize) {
            DEBUG_PRINT("[tests]: CC2: Wrong SA9 size: %d, %d\n", pA1->PayloadSize, pA2->PayloadSize);
            return false;
        }
        if (memcmp(pA1->Payload.pData, pA2->Payload.pData, pA1->PayloadSize) != 0) {
            DEBUG_PRINT("[tests]: CC2: Different SA9\n");
            return false;
        }
    }
    // SA10
    ...
    if ((pA1->Payload.pData == NULL) || (pA2->Payload.pData == NULL)) {
        if (pA1->Payload.pData != pA2->Payload.pData) {
            DEBUG_PRINT("[tests]: CC2: Mismatch absence of SA10\n");
            return false;
        }
    } else {
        if (strcmp((char *)pA1->Payload.pData, (char *)pA2->Payload.pData) != 0) {
            DEBUG_PRINT("[tests]: CC2: Different SA10: %s, %s\n", (char *)pA1->Payload.pData, (char *)pA2->Payload.pData);
            return false;
        }
    }
    return true;
...
static bool compare_valid_cc5(const lcsf_valid_cmd_t *p1, const lcsf_valid_cmd_t *p2) {
    ...
    if ((p1->CmdId != LCSF_TEST_CMD_ID_CC5) || (p2->CmdId != LCSF_TEST_CMD_ID_CC5)) {
        DEBUG_PRINT("[tests]: CC5: Wrong command id: %d, %d\n", p1->CmdId, p2->CmdId);
        return false;
    }
    if ((p1->pAttArray == NULL) || (p2->pAttArray == NULL)) {
        DEBUG_PRINT("[tests]: CC5: Missing attributes\n");
        return false;
    }
    // SA2
    lcsf_valid_att_t *pA1 = &p1->pAttArray[0];
    lcsf_valid_att_t *pA2 = &p2->pAttArray[0];
    if ((pA1->Payload.pData == NULL) || (pA2->Payload.pData == NULL)) {
        DEBUG_PRINT("[tests]: CC5: Missing SA2\n");
        return false;
    }
    if (memcmp(pA1->Payload.pData, pA2->Payload.pData, sizeof(uint16_t)) != 0) {
        DEBUG_PRINT("[tests]: CC5: Different SA2: %d, %d\n", *(uint16_t *)pA1->Payload.pData, *(uint16_t *)pA2->Payload.pData);
        return false;
    }
    // CA5
    pA1 = &p1->pAttArray[1];
    ...
    if ((pA1->Payload.pSubAttArray == NULL) || (pA2->Payload.pSubAttArray == NULL)) {
        DEBUG_PRINT("[tests]: CC5: Missing CA5\n");
        return false;
    }
    // CA5_SA1
    ...
    if (memcmp(pA1->Payload.pData, pA2->Payload.pData, sizeof(uint8_t)) != 0) {
        DEBUG_PRINT("[tests]: CC5_CA5: Different SA1: %d, %d\n", *(uint8_t *)pA1->Payload.pData, *(uint8_t *)pA2->Payload.pData);
        return false;
    }
    // CA5_SA2
    ...
    if (memcmp(pA1->Payload.pData, pA2->Payload.pData, sizeof(uint16_t)) != 0) {
        DEBUG_PRINT("[tests]: CC5_CA5: Different SA2: %d, %d\n", *(uint16_t *)pA1->Payload.pData, *(uint16_t *)pA2->Payload.pData);
        return false;
    }
    // CA5_SA3
    ...
    if ((pA1->Payload.pData == NULL) || (pA2->Payload.pData == NULL)) {
        if (pA1->Payload.pData != pA2->Payload.pData) {
            DEBUG_PRINT("[tests]: CC5_CA3: Mismatch absence of SA3\n");
            return false;
        }
    } else {
        if (memcmp(pA1->Payload.pData, pA2->Payload.pData, sizeof(uint32_t)) != 0) {
            DEBUG_PRINT("[tests]: CC5_CA5: Different SA3: %d, %d\n", *(uint32_t *)pA1->Payload.pData, *(uint32_t *)pA2->Payload.pData);
            return false;
        }
    }
    // CA6
    ...
    if ((pA1->Payload.pSubAttArray == NULL) || (pA2->Payload.pSubAttArray == NULL)) {
        if (pA1->Payload.pSubAttArray != pA2->Payload.pSubAttArray) {
            DEBUG_PRINT("[tests]: CC5: Mismatch absence of CA6\n");
            return false;
        }
    } else {
        // CA6_SA1
        ...
        if ((pA1->Payload.pData == NULL) || (pA2->Payload.pData == NULL)) {
            if (pA1->Payload.pData != pA2->Payload.pData) {
                DEBUG_PRINT("[tests]: CC5_CA6: Mismatch absence of SA1\n");
                return false;
            }
        } else {
            if (memcmp(pA1->Payload.pData, pA2->Payload.pData, sizeof(uint8_t)) != 0) {
                DEBUG_PRINT("[tests]: CC5_CA6: Different SA1: %d, %d\n", *(uint8_t *)pA1->Payload.pData, *(uint8_t *)pA2->Payload.pData);
                return false;
            }
        }
        // CA6_CA7
        ...
        if ((pA1->Payload.pSubAttArray == NULL) || (pA2->Payload.pSubAttArray == NULL)) {
            DEBUG_PRINT("[tests]: CC5_CA6: Missing CA7\n");
            return false;
        }
        // CA7_SA1
        ...
        if ((pA1->Payload.pData == NULL) || (pA2->Payload.pData == NULL)) {
            if (pA1->Payload.pData != pA2->Payload.pData) {
                DEBUG_PRINT("[tests]: CC5_CA7: Mismatch absence of SA1\n");
                return false;
            }
        } else {
            if (memcmp(pA1->Payload.pData, pA2->Payload.pData, sizeof(uint8_t)) != 0) {
                DEBUG_PRINT("[tests]: CC5_CA7: Different SA1: %d, %d\n", *(uint8_t *)pA1->Payload.pData, *(uint8_t *)pA2->Payload.pData);
                return false;
            }
        }
        // CA7_CA8
        ...
        if ((pA1->Payload.pSubAttArray == NULL) || (pA2->Payload.pSubAttArray == NULL)) {
            DEBUG_PRINT("[tests]: CC5_CA8: Missing CA4\n");
            return false;
        }
        // CA8_SA4
        ...
        if (pA1->PayloadSize != pA2->PayloadSize) {
            DEBUG_PRINT("[tests]: CC5_CA8: Wrong SA4 size: %d, %d\n", pA1->PayloadSize, pA2->PayloadSize);
            return false;
        }
        if ((pA1->Payload.pData == NULL) || (pA2->Payload.pData == NULL)) {
            DEBUG_PRINT("[tests]: CC5_CA8: Missing SA4\n");
            return false;
        }
        if (memcmp(pA1->Payload.pData, pA2->Payload.pData, pA1->PayloadSize) != 0) {
            DEBUG_PRINT("[tests]: CC5_CA8: Different SA4\n");
            return false;
        }
    }
    return true;
...
int LCSF_ValidatorEncode(uint_fast16_t protId, const lcsf_valid_cmd_t *pCommand, uint8_t *pBuffer, size_t buffSize) {
    DEBUG_PRINT("[tests]: Encode cmd: %d\n", pCommand->CmdId);
